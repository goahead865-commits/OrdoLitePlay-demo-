...
def main() -> None:
    ...
    b1, b2, _bsp = st.columns([0.18, 0.22, 0.60])
    with b1:
        if st.button(_t("設定採購單價"), type="primary", use_container_width=True):
            ...
            if not st.session_state.c01_selected_item_id:
                st.warning(_t("請先在表格勾選一筆原料，再按設定採購單價。"))
            else:
    ...
    if st.session_state.c01_form_open:
        ...
        mode_key = f"edit_{int(selected_id)}" if is_edit else "new"
        if not is_edit:
            default = {"item_code": get_next_material_code()}
        ...
        item_code_default = _safe_str(default.get("item_code")) if is_edit else get_next_material_code()
        f1, f2, f3, f4 = st.columns([0.22, 0.32, 0.23, 0.23])
        with f1:
            item_code = st.text_input("料號 *", value=_safe_str(default.get("item_code")), disabled=(not is_edit), key=f"c01_item_code_{mode_key}")
        with f2:
